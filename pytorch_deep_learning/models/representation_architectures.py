...
class PyramidConv8D:
    def __init__(self, n_channels, view_dim, rep_dim=256):
        """
        Network that generates a condensed representation
        vector from a joint input of image and viewpoint.
        ...
        :param n_channels: number of color channels in input image
        :param view_dim: dimensions of the viewpoint vector
        :param rep_dim: dimensions of representation
        """
        super(PyramidRepresentation, self).__init__()
        # Final representation size
        self.rep_dim = k = rep_dim

        self.conv1 = nn.Conv2d(n_channels + view_dim, k//8, kernel_size=2, stride=2)
